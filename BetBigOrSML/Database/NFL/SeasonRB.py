...
season_rb_collection = db['RB Season Statistics']
...
rb_stats = weekly_data[weekly_data['position'] == 'RB']
...
season_stats = rb_stats.groupby('player_id').agg({
...
season_rb_collection.delete_many({})  # Clear existing season stats (if needed)
season_rb_collection.insert_many(season_stats_dict)

print(f"Full season QB stats have been successfully updated in MongoDB at {current_time}.")
